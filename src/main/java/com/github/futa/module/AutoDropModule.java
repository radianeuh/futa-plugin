...
public class AutoDropModule extends BaseModule {
    ...
    public boolean enabledSetting() {
        return PLUGIN_CONFIG.autoDrop.enabled;
    }
    ...
    private void onTick(ClientBotTick event) {
        ...
        if (dropTimer.tick(PLUGIN_CONFIG.autoDrop.delayBetweenDrops)) {
            dropTimer.reset();
            performAutoDrop();
        }
    }
    ...
    private boolean shouldDropItem(int itemId) {
        ...
        boolean isInList = PLUGIN_CONFIG.autoDrop.items.contains(itemName);

        return PLUGIN_CONFIG.autoDrop.whitelistMode ? isInList : !isInList;
    }
}
